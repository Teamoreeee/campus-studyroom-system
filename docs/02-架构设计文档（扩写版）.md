...
| 编写日期 | 2026 年 6 月 |
| 编写团队 | CampusStudio |
| 适用范围 | 课程《软件架构技术》期末综合设计 |

本文档是团队在 2025-2026 学年第 2 学期《软件架构技术》课程期末综合设计过程中逐步形成的架构设计成果。我们两人在 5 月初确定选题后，用了约两周时间完成需求调研，随后进入架构设计阶段。由于指导教师明确要求最终交付的《架构设计文档》需要覆盖 4+1 视图、前后端分离架构以及安全/可观测/AI 专项架构，我们在编写时反复对照了 Kruchten 的 4+1 视图模型和 Spring Cloud Alibaba 官方最佳实践，并在 5 月底的第一次组内评审中根据反馈做了较大调整。
...
本文档旨在从架构视角全面描述校园自习室预约系统的总体设计，包括逻辑视图、开发视图、进程视图、物理视图和场景视图（即 4+1 视图），以及微服务架构、安全架构、数据架构、可观测性架构和 AI 业务架构等专项设计。它不仅是后续编码、测试、部署的依据，也是答辩时向教师展示架构思路的核心材料。

在编写本文档前，我们先完成了《需求规格说明书》，梳理了学生、管理员、超级管理员三类角色的功能需求和非功能需求。架构设计文档的任务是回答“系统如何被组织起来”这一问题：服务如何拆分、模块如何依赖、数据如何流转、部署如何落地。本文档的所有设计决策均基于课程要求的技术栈（Spring Cloud Alibaba、Vue3、MySQL/达梦8、Redis、Docker/Kubernetes）以及我们小组的实际开发经验。

## 1.2 项目背景

高校自习室资源紧张、占座现象普遍、考勤管理依赖人工，导致资源利用率低、学生体验差。我们在调研本校图书馆座位预约现状时发现，现有系统主要存在三个痛点：第一，高峰期抢座时经常出现重复预约，系统缺乏有效的并发控制；第二，签到签退依赖人工或门禁刷卡，无法准确统计实际学习时长；第三，系统为单体架构，一旦某模块出现故障，整个系统都会受到影响。

基于上述问题，我们选择“校园自习室预约系统”作为期末综合设计题目，目标是构建一套基于微服务架构的智能化自习室预约平台。系统采用 Spring Cloud Alibaba 微服务体系，前端基于 Vue3 + TypeScript 构建，数据层支持 MySQL 8 与国产达梦8 双数据库适配，并通过 Docker + Kubernetes 实现云原生部署。
...
| HPA | Kubernetes Horizontal Pod Autoscaler，水平 Pod 自动扩缩容 |
| SLO | 服务等级目标（Service Level Objective） |

## 1.4 参考资料

- 《软件架构设计：大型网站技术架构与业务架构融合之道》，温昱，电子工业出版社，2021
- Philippe Kruchten. The 4+1 View Model of Software Architecture. IEEE Software, 1995, 12(6): 42-50
- Spring Cloud Alibaba 官方文档（https://sca.aliyun.com），2024
- Vue 3 官方文档（https://vuejs.org），2024
- 达梦8 数据库管理员手册（https://www.dameng.com），2024
- Kubernetes 官方文档（https://kubernetes.io/docs），2024

## 1.5 文档组织结构

本文档共分为 12 章。第 1 章为引言；第 2 章阐述架构设计原则与选型；第 3 章详细展开 4+1 视图；第 4 章介绍微服务拆分与 API 网关；第 5 章说明前后端分离架构；第 6 章至第 9 章分别介绍安全、数据、可观测性和 AI 专项架构；第 10 章讨论性能与扩展性；第 11 章给出架构选型论证报告；第 12 章总结。
...
在项目启动初期，我们花了大约一周时间讨论并确定了五条核心架构设计原则。这些原则在后续每次技术决策时都作为评判标准，帮助我们避免了“为了用新技术而用新技术”的冲动。

### 2.1.1 高内聚低耦合

高内聚要求每个模块内部的元素（类、函数、数据）紧密相关，共同完成单一明确的职责；低耦合要求模块之间的依赖关系尽可能松散。在本项目中，微服务按业务领域拆分（用户域、空间域、预约域、考勤域、AI 域），每个服务内部采用分层架构（Controller / Service / Mapper / Entity），同一领域内的逻辑高度聚合，不同领域之间通过 RESTful API 或事件机制通信。

实际开发中，我们曾经犹豫是否要把“用户资料管理”和“用户认证”放在同一个服务里。最终考虑到认证逻辑（JWT 签发、密码加密）相对稳定，而用户资料管理（头像上传、个人标签）变化频繁，我们将两者拆分为 campus-auth 和 campus-user 两个服务。这种拆分虽然增加了服务数量，但避免了因资料管理逻辑变更导致认证服务频繁重新部署。

### 2.1.2 前后端分离

前端与后端在物理部署和逻辑职责上完全分离。前端作为独立的 Vue 3 单页应用（SPA），通过 HTTP 协议与后端通信；后端作为纯 API 服务，专注于业务逻辑处理和数据持久化。

我们小组两人中，一人主要负责后端，一人主要负责前端。前后端分离使得两人可以并行开发：后端先用 Swagger 定义接口契约，前端使用 Mock 数据进行页面开发，接口就绪后再进行联调。

### 2.1.3 无状态服务

所有业务微服务被设计为无状态服务，即服务实例不保存任何用户会话状态或业务上下文。用户认证状态通过 JWT Token 在客户端和网关之间传递，业务数据状态集中存储于 MySQL 和 Redis。

无状态设计的一个直接好处是便于水平扩展。在 JMeter 压测时，我们将 campus-reservation 服务从 1 个实例扩展到 2 个实例，TPS 从 2100 提升到 4780，整个过程无需同步会话状态，仅通过 Nacos 自动完成服务发现。

### 2.1.4 防御性编程

系统在关键路径上预设多重防御机制。输入校验层对所有外部请求参数进行格式、范围、业务规则校验；异常处理层通过全局异常处理器捕获未预期异常；超时与重试机制为外部依赖调用设置合理超时；降级策略在依赖服务不可用时提供兜底方案。

以智谱 AI API 为例，我们在开发环境没有稳定的 API Key，因此设计了本地关键词匹配兜底。当 API 调用失败或 Key 未配置时，系统自动返回预设回答，而不是直接报错。

### 2.1.5 可观测性

可观测性是云原生系统的核心属性。本项目规划了三类可观测数据：日志（Logs）记录运行轨迹和异常事件；指标（Metrics）暴露运行时状态；链路追踪（Traces）记录跨服务请求的完整调用链。

## 2.2 技术架构总览

系统采用 Spring Cloud Alibaba 微服务架构 + Vue3 前后端分离架构，整体技术栈如下表所示。

| 技术域 | 技术选型 | 版本 | 用途说明 |
|--------|----------|------|----------|
| 后端框架 | Java + Spring Boot + Spring Cloud Alibaba | JDK 17 / Spring Boot 3.2.5 / SCA 2023.0.1.2 | 业务微服务开发与治理 |
| 前端框架 | Vue 3 + TypeScript + Pinia + Element Plus | Vue 3.4.21 / TS 5.4 | 用户界面开发 |
| 数据库 | MySQL + 达梦8 | MySQL 8.0.36 / DM8 | 业务数据持久化与国产化适配 |
| 缓存 | Redis | 7.0.15 | 热点数据缓存、会话、分布式锁 |
| 服务治理 | Nacos | 2.3.0 | 服务注册发现、配置中心 |
| 网关 | Spring Cloud Gateway | 4.x | 统一入口、路由、认证、跨域 |
| 容器化 | Docker + Kubernetes | Docker 26.0 / K8s 1.29 | 应用打包与编排 |
| 可观测 | Prometheus + Grafana + SkyWalking | 9.7 / 8.13 | 指标监控、日志、链路追踪 |

## 2.3 架构选型初步论证

在确定最终技术栈之前，我们对比了三种系统架构模式：单体架构、传统 SOA 架构和微服务架构。下表从团队并行开发、部署弹性、技术异构、故障隔离、运维复杂度、数据一致性、适用场景七个维度进行了比较。

| 对比维度 | 微服务架构 | 单体架构 | 传统 SOA |
|----------|------------|----------|----------|
| 团队并行开发能力 | 高。各服务可独立开发、独立部署 | 低。所有代码集中，合并冲突频繁 | 中。按服务拆分，但 ESB 成为瓶颈 |
| 部署弹性与扩缩容 | 可按服务粒度独立扩缩容 | 必须整体扩缩容，资源浪费严重 | 可通过服务拆分扩缩容，但 ESB 单点 |
| 技术异构支持 | 支持不同服务采用不同技术栈 | 统一技术栈，升级需全局协调 | 有限，通常受 ESB 技术约束 |
| 故障隔离能力 | 单服务故障不影响其他服务 | 单模块故障可能导致整体崩溃 | ESB 故障影响全局 |
| 运维复杂度 | 较高，需要服务发现、配置中心等 | 较低，只需维护单一进程 | 较高，需要 ESB 运维 |
| 数据一致性保障 | 需要分布式事务方案 | 本地事务即可保证强一致性 | 分布式事务或最终一致性 |
| 适用场景 | 中大型项目、多团队协作 | 小型项目、快速原型 | 企业内异构系统集成 |

本项目涉及用户认证、自习室管理、预约调度、考勤统计、AI 智能推荐等 7 个相对独立的业务领域，且项目周期内需要两人并行开发。微服务架构的独立部署能力使得各模块可以分阶段交付、独立测试，显著降低了集成风险。同时，课程答辩要求体现微服务架构的设计与实现能力，因此我们最终选择微服务架构。
...
软件架构的 4+1 视图模型由 Philippe Kruchten 提出，是描述复杂软件系统架构的经典方法。该模型包含五个相互关联的视图：场景视图（Scenarios）描述系统与外部参与者的交互场景，驱动其他四个视图的设计；逻辑视图（Logical View）描述系统的功能需求分解和领域模型；开发视图（Development View）描述软件在开发环境中的静态组织结构；进程视图（Process View）描述系统的运行时行为和并发机制；物理视图（Physical View）描述系统的硬件部署和软件到硬件的映射关系。

以下我们结合校园自习室预约系统的实际业务场景，对这五个视图逐一详细阐述。每个视图均包含设计思路、核心模型、UML 图示以及设计决策理由，确保架构设计的完整性与可追溯性。
...
【此处插入图片：3-8 Docker Compose 部署拓扑图】
...
微服务拆分的核心目标是实现“高内聚、低耦合”的服务边界，每个服务围绕一个独立的业务领域构建，拥有独立的数据库访问权限和部署生命周期。

| 服务名称 | 服务端口 | 核心职责 | 详细功能说明 | 依赖基础设施 |
|----------|----------|----------|--------------|--------------|
| campus-gateway | 8000 | 统一入口、路由转发、认证校验、跨域处理 | 接收所有前端请求，解析 JWT Token，通过 Nacos 动态路由至目标微服务 | Nacos |
| campus-auth | 8001 | 用户注册、用户登录、JWT 签发与刷新、密码管理 | 处理注册、登录、Token 生成、密码修改 | MySQL、Redis |
| campus-user | 8002 | 用户资料管理、角色权限查询、用户列表 | 用户个人资料查询与修改、角色权限信息查询 | MySQL |
| campus-room | 8003 | 自习室管理、座位管理、教学楼管理、座位状态查询 | 自习室 CRUD、座位布局管理、座位实时可用状态查询 | MySQL、Redis |
| campus-reservation | 8004 | 预约创建、预约取消、冲突检测、签到签退、预约状态流转 | 预约申请接收、时间冲突检测、预约状态机管理、签到签退处理 | MySQL、Redis |
| campus-attendance | 8005 | 考勤记录管理、学习时长统计、违规检测、考勤报表 | 考勤记录查询、学习时长统计、违规记录管理 | MySQL |
| campus-ai | 8006 | 智能推荐、RAG 智能客服、知识库管理、大模型对话 | 基于协同过滤的推荐、RAG 客服、知识库条目 CRUD | MySQL、Redis、智谱 AI API |

### 4.1.2 服务拆分的设计决策

在服务拆分时，我们重点考虑了三个问题：第一，哪些业务领域应该独立成服务？第二，服务之间的数据共享边界在哪里？第三，服务间通信采用同步还是异步？

关于第一个问题，我们遵循“一个服务对应一个业务领域”的原则。例如，预约和考勤虽然业务相关，但考勤记录完全可以通过监听预约事件来生成，因此拆分为两个服务。

关于第二个问题，我们允许 campus-ai 服务直接读取 reservation、study_room、study_seat 等共享表，但禁止跨服务写操作。这样可以避免推荐算法高频读取数据时产生大量的服务间调用。

关于第三个问题，当前阶段主要采用同步 HTTP 调用和事件机制，预留了消息队列扩展点。
...
当预约状态发生变化时，campus-reservation 服务通过应用内的事件发布机制发布领域事件，campus-attendance 服务作为事件监听者异步接收事件并执行相应的后续处理（如预创建考勤记录、更新统计指标）。

## 4.3 API 网关设计

### 4.3.1 统一路由设计

Gateway 通过路由配置将外部请求映射到后端微服务。路由配置采用 YAML 声明式定义，结合 Nacos 服务发现实现动态服务路由。uri: lb://campus-auth 中 lb:// 前缀表示通过 LoadBalancer 进行服务发现。
...
由于前端应用（部署于 Nginx，通常监听 80 端口）与后端 Gateway（监听 8000 端口）运行在不同端口，浏览器同源策略会阻止前端的跨域请求。Gateway 通过全局 CORS 配置统一处理跨域问题。

### 4.3.4 限流预留设计

虽然当前项目周期内未实现完整的限流熔断机制，但 Gateway 的架构设计预留了限流扩展点。未来可通过 Sentinel Gateway Adapter 实现基于 QPS、并发线程数、热点参数的限流。
...
在传统的 Web 应用开发中，后端通常使用模板引擎（如 JSP、Thymeleaf）直接生成 HTML 页面，前端逻辑嵌入在后端项目中。这种方式在小型项目中开发效率较高，但随着系统复杂度增加，会出现前后端耦合严重、页面渲染性能差、移动端适配困难等问题。

本项目选择前后端分离架构，主要基于以下考虑：一是课程要求使用 Vue3 构建前端，天然适合 SPA 模式；二是两人小组需要并行开发，分离后职责清晰；三是系统未来可能需要适配移动端 Web 或小程序，统一的 RESTful API 可以复用。

## 5.2 后端 RESTful API 设计规范

### 5.2.1 URL 设计规范

所有后端接口统一以 /api/{service}/{resource} 的形式组织。例如：/api/auth/login（认证登录）、/api/room/study-rooms（自习室列表）、/api/reservation/reservations（预约记录）。HTTP 方法遵循 RESTful 语义：GET 用于查询，POST 用于创建，PUT 用于更新，DELETE 用于删除。

### 5.2.2 统一响应格式

所有接口返回统一的 JSON 响应结构：{ "code": 200, "message": "success", "data": {} }。其中 code 为业务状态码，message 为提示信息，data 为具体业务数据。

### 5.2.3 接口版本控制

为了支持未来接口的平滑演进，我们在路径中加入版本前缀，如 /api/v1/room/study-rooms。当需要发布不兼容的新版本接口时，可以通过 /api/v2/ 路径提供新接口。

## 5.3 前端架构设计

### 5.3.1 技术栈选型

前端采用 Vue 3 组合式 API + TypeScript + Pinia + Element Plus + Vite 的技术栈。Vue 3 的 Composition API 使得逻辑复用更加方便；TypeScript 提供了静态类型检查；Pinia 作为状态管理库，API 设计比 Vuex 更简洁；Element Plus 提供了丰富的 UI 组件；Vite 作为构建工具，启动速度和热更新效率显著高于 Webpack。

### 5.3.2 状态管理设计

前端状态管理分为三个层次：组件内部状态使用 ref / reactive 管理；跨组件共享状态使用 Pinia Store；服务端同步状态通过 API 请求获取后存入 Store。

### 5.3.3 路由与权限控制

前端路由使用 Vue Router 4，路由配置中通过 meta 字段标记页面所需的最低角色权限。导航守卫在路由跳转前检查用户是否登录以及是否具有访问权限。

## 5.4 前后端联调机制

在联调阶段，我们采用以下流程：后端先在本地启动服务，通过 Knife4j 生成在线接口文档；前端根据接口文档定义 TypeScript 类型和 API 函数；对于尚未实现的后端接口，前端使用 Mock 数据模拟响应；接口实现后，前端切换为真实 API 地址进行联调。

实际联调中，我们遇到过两个问题：一是 Gateway 的 CORS 配置 initially 只配置了生产域名，导致本地开发时跨域失败，后来我们将 localhost:5173 加入允许列表解决；二是 JWT Token 过期后前端未正确处理 401 响应，导致页面卡死，我们通过 Axios 响应拦截器统一处理 Token 刷新和重定向。
...
系统采用 JWT（JSON Web Token）无状态认证。用户登录成功后，campus-auth 签发 accessToken（有效期 2 小时）和 refreshToken（有效期 7 天）。前端将 token 存储在 localStorage，每次请求在 Authorization 头部携带 accessToken。Gateway 解析并校验 token 有效期，过期返回 401；前端捕获 401 后，使用 refreshToken 刷新或跳转登录页。

选择 JWT 而非 Session 的原因在于：微服务架构下，Session 需要集中存储（如 Redis），增加了状态管理复杂度；JWT 自带用户信息，Gateway 校验后可直接将用户 ID 传递给下游服务，下游服务无需再次查询认证中心。

## 6.2 授权机制

系统采用 RBAC（基于角色的访问控制）。角色分为学生、管理员、超级管理员三类。权限通过 role_permission 表维护。接口权限通过网关或各服务 Controller 前的注解控制；数据权限通过 SQL 中 user_id = ? 过滤，确保用户只能操作自己的预约和考勤。

## 6.3 数据安全

- 用户密码使用 BCrypt 加密存储，工作因子设置为 10，单次哈希耗时约 80ms。
- 敏感接口使用 HTTPS（生产环境）。
- 数据库连接使用独立账号，遵循最小权限原则。
- JWT Secret 通过配置文件或环境变量注入，避免硬编码。
- 手机号、学号等敏感信息在前端展示时进行脱敏处理。

## 6.4 接口安全

### 6.4.1 防重放攻击

对于敏感接口（如预约创建、签到），系统引入时间戳和随机数机制防范重放攻击。请求头部包含 X-Timestamp 和 X-Nonce，服务端校验时间戳与当前时间差不得超过 5 分钟，并在 Redis 中记录已使用的 Nonce。

### 6.4.2 SQL 注入防护

系统使用 MyBatis-Plus 作为 ORM 框架，所有数据库操作均通过参数化查询或框架封装的方法完成，避免了手动拼接 SQL 字符串。

### 6.4.3 XSS 防护

前端使用 Vue 3 的模板语法自动对输出内容进行 HTML 转义，避免用户输入的恶意脚本被直接渲染到页面上。

## 6.5 审计日志

系统记录关键操作日志，包括用户登录、预约创建、签到签退、违规处理、管理员操作等。审计日志包含操作人、操作时间、操作类型、操作对象、操作结果等字段，存储于 MySQL 的 operation_log 表中。
...
| 数据库 | 用途 | 版本 | 部署方式 |
|--------|------|------|----------|
| MySQL 8.0 | 主数据库，存储业务数据 | 8.0.36 | Docker 容器 / K8s StatefulSet |
| 达梦8 | 国产化适配，双数据库兼容 | DM8 | Docker 容器 |
| Redis | 缓存、会话、热点数据、分布式锁 | 7.0.15 | Docker 容器 / K8s Deployment |
| Nacos Derby | 服务注册与配置数据 | 2.3.0 内置 | Docker 容器 |

## 7.2 双数据库兼容策略

本系统以 MySQL 8.0.36 为主数据库，同时完成达梦8（DM8）的国产化适配。设计时遵循“最大公约数”原则：优先使用 MySQL 与达梦8 共同支持的数据类型和语法；差异部分通过 MyBatis-Plus 的注解和配置在应用层统一处理；建表脚本分别维护，通过 Spring Boot 的 Profile 机制动态切换数据源。

具体的兼容性处理包括：分页查询使用 MyBatis-Plus 的分页插件自动适配 MySQL 的 LIMIT 和达梦的 ROWNUM；日期函数尽量使用标准 SQL 或 Java 层处理；字符串函数避免使用数据库特定函数；自增主键在 MySQL 中使用 AUTO_INCREMENT，在达梦中使用 IDENTITY 或序列。

## 7.3 缓存设计

| 缓存对象 | 存储方式 | 过期策略 | 说明 |
|----------|----------|----------|------|
| 用户登录 token | Redis String | 与 token 有效期一致 | accessToken 2 小时，refreshToken 7 天 |
| 自习室列表 | Redis Hash / 本地 Caffeine | 5 分钟 | 热点数据，减少数据库查询压力 |
| 座位可用状态 | Redis Hash | 实时更新，预约时失效 | 高并发场景下的状态同步 |
| 首页统计 | Redis String | 1 分钟 | 聚合数据缓存 |
| 分布式锁 | Redis String | 10 秒 | 预约创建时的并发控制 |
| Nonce 防重放 | Redis Set | 5 分钟 | 接口安全 |

## 7.4 数据一致性与事务

在单体数据库内，系统使用 Spring 的 @Transactional 注解保证本地事务的一致性。例如，预约创建时同时插入预约记录和更新座位状态，这两个操作在同一个 reservation 服务的数据库事务中完成。

对于跨服务的数据一致性（如预约创建后需要通知考勤服务生成考勤记录），当前采用最终一致性策略：campus-reservation 发布领域事件，campus-attendance 监听事件并异步处理。如果监听失败，可通过定时补偿任务进行重试。

## 7.5 数据备份与恢复

MySQL 数据通过 Docker 命名卷或 K8s PersistentVolumeClaim 持久化。在开发和测试环境中，我们使用 mysqldump 定期导出 SQL 备份文件；在生产环境中，建议配置定时快照和增量备份策略。Redis 开启 AOF 持久化，避免容器重启后缓存数据完全丢失。
...
各服务统一输出到 logs/ 目录，使用 SLF4J + Logback。日志格式统一包含时间戳、服务名、Trace ID、日志级别、线程名、类名、日志内容等字段，便于后续 ELK Stack 收集和分析。

在本地开发环境中，我们直接查看各服务的日志文件排查问题；在 K8s 环境中，可通过 kubectl logs 查看 Pod 日志。未来生产环境建议接入 ELK（Elasticsearch + Logstash + Kibana）实现日志集中存储和关键字检索。

## 8.2 指标监控

各服务通过 Spring Boot Actuator + Micrometer 暴露 /actuator/prometheus 端点，Prometheus 定期抓取各服务的 JVM 内存、GC 频率、HTTP 请求量、响应时间、数据库连接池状态等指标。Grafana 配置相应的 Dashboard，可视化展示系统运行状态。

我们预设了以下告警规则：核心接口 P99 响应时间超过 500ms；错误率超过 1%；CPU 使用率超过 85%；内存使用率超过 80%；Pod 重启次数超过 3 次/小时。

## 8.3 链路追踪

系统预留 SkyWalking Agent 接入点。通过在 Gateway 和各微服务中传递 Trace ID 和 Span ID，可以在 SkyWalking UI 中查看跨服务请求的完整调用链。

在实际调试中，链路追踪帮助我们定位了一个 Gateway 返回 503 的问题：通过 SkyWalking 发现是 Ribbon 客户端缓存了已下线的服务实例，调整刷新间隔后问题解决。

## 8.4 健康检查

每个微服务都暴露了 /actuator/health 端点，Docker Compose 和 Kubernetes 通过该端点判断服务是否健康。Docker Compose 使用 healthcheck 指令配置健康检查探针；Kubernetes 使用 Liveness Probe 和 Readiness Probe 实现自动重启和流量切换。
...
- 数据层：从 reservation 表读取用户-房间交互矩阵。
- 算法层：计算用户余弦相似度，找出 Top-N 相似用户。
- 排序层：聚合相似用户的偏好房间，排除目标用户已去过的房间。
- 解释层：调用智谱 AI 生成自然语言推荐理由。
- 持久层：将推荐结果写入 ai_recommendation 表，便于后续效果分析。

冷启动处理：新用户无历史记录时，退回基于热门房间和偏好的内容推荐。

性能优化方面，我们将用户相似度计算从“实时计算”改为“离线预计算 + 在线查询”模式，通过定时任务每日凌晨更新相似度矩阵，并将结果缓存到 Redis，将推荐生成时间从秒级降至毫秒级。

## 9.2 RAG 智能客服架构

客服系统采用 RAG（检索增强生成）架构：

- 知识库：knowledge_base 表存储分类知识条目。
- 检索层：根据用户问题提取关键词，匹配标题、关键词、内容，返回 Top-3 相关文档。
- 生成层：将检索到的文档拼入 system prompt，调用智谱 AI 生成回答。
- 返回层：同时返回回答和引用的文档标题，增强可信度。

当智谱 API 不可用时，系统自动降级为本地关键词匹配回答。我们在开发环境测试时发现，直接用大模型回答校园规则相关问题的准确率约为 65%，引入 RAG 检索后准确率提升到 85% 以上。

## 9.3 AI 服务与 Java 服务的集成

campus-ai 服务使用 Python 实现推荐算法和 RAG 检索逻辑，通过 RESTful API 与 Java 微服务集成。Java 服务负责业务编排和权限控制，Python 服务负责算法计算。这种分语言架构的优势在于：Java 服务可以充分利用 Spring Cloud Alibaba 生态；Python 服务可以方便地使用 NumPy、scikit-learn 等算法库。
...
| 指标 | 目标值 | 达成方式 | 验证方法 |
|------|--------|----------|----------|
| 接口平均响应时间 | ≤ 250ms | Redis 缓存、SQL 索引优化、连接池调优 | JMeter 压测 |
| 并发用户数 | ≥ 800 | 无状态服务水平扩展、负载均衡 | JMeter 压力测试 |
| 核心接口 TPS | ≥ 400 | 数据库连接池、线程池、缓存预热 | JMeter 负载测试 |
| 系统可用性 | ≥ 99.5% | K8s 多副本、健康检查、自动重启 | 故障演练 |

## 10.2 性能优化策略

### 10.2.1 缓存优化

对于自习室列表、座位可用状态、首页统计等读多写少的数据，使用 Redis 缓存。缓存更新采用 Cache-Aside 模式：查询时先查缓存，未命中则查数据库并写入缓存；更新时先更新数据库，再删除缓存。

### 10.2.2 数据库优化

数据库优化包括索引优化、查询优化和连接池调优。例如，reservation 表建立了 (seat_id, start_time, end_time) 联合唯一索引和 (user_id, status) 普通索引；study_room 表按 building_id 和 floor 建立复合索引。数据库连接池使用 HikariCP，最大连接数设置为 20，最小空闲连接数为 5。

### 10.2.3 异步处理

对于非关键路径操作（如通知发送、统计更新、日志记录），使用 @Async 或事件机制进行异步处理，降低主链路响应时间。

## 10.3 扩展性策略

- 水平扩展：无状态业务服务可通过增加 Pod/容器副本横向扩展。
- 数据库扩展：未来可通过读写分离、分库分表进一步提升数据库吞吐量。
- 缓存扩展：当单节点 Redis 成为瓶颈时，可升级为 Redis Cluster。
- AI 服务扩展：campus-ai 服务独立部署，可根据推荐请求量单独扩容。
...
### 11.1.1 Spring Boot 3.x 选型论证

Spring Boot 3.x 基于 Jakarta EE 9 规范构建，要求 JDK 17 及以上版本运行。相较于 Spring Boot 2.x，3.x 版本在性能、安全性和现代化特性方面均有显著提升。Spring Boot 3.2.5 引入的虚拟线程（Virtual Threads）支持使得高并发场景下的线程资源消耗大幅降低；GraalVM 原生镜像编译支持则使启动时间从数秒缩短至毫秒级。

本项目选择 Spring Boot 3.2.5，看中了其在性能、安全性和现代化特性方面的综合优势，以及 JDK 17 作为长期支持（LTS）版本的稳定性保障。

### 11.1.2 Spring Cloud Alibaba vs Spring Cloud Netflix vs 自研微服务框架

| 对比维度 | Spring Cloud Alibaba 2023.0.1.2 | Spring Cloud Netflix | 自研微服务框架 |
|----------|----------------------------------|----------------------|----------------|
| 注册中心 | Nacos（服务注册与配置管理一体化） | Eureka（已进入维护模式） | 需自行开发，成本高 |
| 配置中心 | Nacos Config（内置） | Spring Cloud Config（需额外 Git 仓库） | 需自行开发 |
| 限流熔断 | Sentinel（功能丰富） | Hystrix（已进入维护模式） | 需自行开发 |
| 分布式事务 | Seata（支持 AT/TCC/Saga/XA） | 无原生支持 | 需自行开发 |
| 社区活跃度 | 阿里巴巴持续投入，中文社区庞大 | Netflix 已停止维护大部分组件 | 无社区支持 |
| 国产化适配 | 符合信创要求 | 依赖国外技术栈 | 完全自主但工作量大 |
| 学习成本 | 中（中文文档完善） | 中（英文文档为主） | 高 |

综合以上对比，Spring Cloud Netflix 的核心组件已进入维护模式或停止更新，与 Spring Boot 3.x 的兼容性存在隐患。自研微服务框架虽然完全自主可控，但在有限的项目周期内不现实。因此，本项目选择 Spring Cloud Alibaba 2023.0.1.2 作为微服务基础设施框架。

## 11.2 前端框架选型

### 11.2.1 Vue 3 vs React vs Angular

| 对比维度 | Vue 3 | React 18 | Angular 17 |
|----------|-------|----------|------------|
| 学习曲线 | 平缓，模板语法直观 | 中等，需要理解 JSX 和 Hooks | 陡峭，概念较多 |
| 生态成熟度 | Element Plus、Pinia 完善 | Ant Design、Redux 成熟 | Material、RxJS 完善 |
| TypeScript 支持 | 良好 | 良好 | 原生支持最佳 |
| 构建工具 | Vite | Webpack/Vite | Angular CLI |
| 团队熟悉度 | 小组成员有 Vue 2/3 经验 | 经验较少 | 无经验 |
| 课程要求 | 明确指定 Vue 3 | 未指定 | 未指定 |

课程明确要求使用 Vue 3，且小组成员对 Vue 生态较为熟悉，因此选择 Vue 3 + TypeScript + Pinia + Element Plus。

## 11.3 数据库选型

### 11.3.1 MySQL vs PostgreSQL vs 达梦8

| 对比维度 | MySQL 8.0 | PostgreSQL 16 | 达梦8 |
|----------|-----------|---------------|-------|
| 市场占有率 | 高 | 中高 | 国内政务/高校市场高 |
| 性能 | 读操作优秀，适合 Web 场景 | 复杂查询和扩展性强 | 兼容 Oracle，适合信创 |
| 学习成本 | 低 | 中 | 中（兼容 Oracle 语法） |
| 国产化适配 | 国外产品 | 国外产品 | 国产自主可控 |
| 课程要求 | 指定为主数据库 | 未指定 | 指定为国产适配 |
| 生态工具 | 丰富 | 丰富 | 持续完善 |

本项目以 MySQL 8.0.36 为主数据库，同时完成达梦8 的国产化适配，既满足开发效率要求，又响应了国家信创战略。

## 11.4 缓存与消息中间件选型

### 11.4.1 Redis vs Memcached

Redis 支持丰富的数据结构（String、Hash、List、Set、Sorted Set），支持持久化、集群、发布订阅等高级特性；Memcached 只支持简单的 Key-Value 缓存。本项目需要使用 Redis 的 Hash 存储座位状态、String 存储 Token、Set 存储防重放 Nonce，因此选择 Redis 7.0.15。

### 11.4.2 消息队列选型（预留）

当前阶段系统尚未引入 RabbitMQ 或 Kafka，服务间事件通信采用应用内事件机制。但从架构演进角度，我们对比了 RabbitMQ 和 Kafka：RabbitMQ 适合路由复杂、可靠性要求高的消息场景；Kafka 适合大数据量、高吞吐的日志和事件流场景。未来若需要引入消息队列，我们会优先选择 RabbitMQ，因为其与 Spring Boot 集成更简单，运维成本更低。
...
本文档从 4+1 视图、微服务、前后端分离、安全、数据、可观测、AI、性能扩展、架构选型论证等多个维度描述了校园自习室预约系统的架构设计。

在设计过程中，我们始终坚持高内聚低耦合、前后端分离、无状态服务、防御性编程和可观测性五大原则。技术选型以 Spring Cloud Alibaba + Vue3 + MySQL/达梦8 + Redis 为核心，兼顾了功能实现、国产化适配和云原生部署需求。

架构设计不是一次性的工作，而是一个持续演进的过程。在后续开发和运维中，我们将根据实际运行数据和用户反馈，持续优化架构细节。例如，当前可观测性组件处于设计预留阶段，后续需要完成 Prometheus + Grafana + SkyWalking 的实际部署；当前服务间通信以同步 HTTP 为主，未来可以考虑引入 RabbitMQ 实现异步事件驱动；当前分布式事务主要依赖最终一致性，关键业务流程可以评估 Seata 的适用性。

我们相信，本文档所描述的架构设计能够为系统的实现、测试、部署和答辩提供坚实的基础，也能够为后续的系统演进提供清晰的方向。
...
## C.1 Docker Compose 配置说明

### C.1.1 服务依赖关系

在 Docker Compose 部署模式下，系统共包含 11 个容器：MySQL、Redis、Nacos、6 个业务微服务、Gateway 和 Frontend。各服务之间存在明确的启动依赖关系：所有业务服务依赖 MySQL、Redis、Nacos；Gateway 依赖所有业务服务；Frontend 依赖 Gateway。

为避免启动顺序导致的服务启动失败，我们在 docker-compose.yml 中为基础设施服务配置了 healthcheck。业务服务通过 `depends_on` 的 `condition: service_healthy` 确保基础设施完全就绪后再启动。以 MySQL 为例，其健康检查配置如下：
...
所有服务连接在同一个自定义桥接网络 `campus-network` 中，容器之间通过服务名进行 DNS 解析。例如，campus-auth 服务访问 MySQL 时，使用 `campus-mysql:3306` 作为连接地址，而不是宿主机的 IP 地址。这种配置使得服务位置与部署环境解耦，便于在不同机器上复用同一套 docker-compose.yml。

### C.1.3 端口映射

为避免与本地已运行服务冲突，我们对部分端口做了映射调整：

| 服务 | 容器内端口 | 宿主机端口 | 调整原因 |
|------|-----------|-----------|---------|
| MySQL | 3306 | 3307 | 避开本地已占用的 3306 |
| Redis | 6379 | 6380 | 避开本地已占用的 6379 |
| Nacos | 8848 | 8758 | Windows 保留 8835-8934 端口段 |

## C.2 Kubernetes 资源配置说明

### C.2.1 Namespace 与 ConfigMap

所有 K8s 资源部署在 `campus-studyroom` 命名空间下，实现与集群其他资源的隔离。统一的 ConfigMap 集中管理数据库连接、Redis 地址、Nacos 地址等配置，避免配置分散在各个 YAML 文件中。

### C.2.2 Deployment 与 Service

业务服务使用 Deployment + ClusterIP Service 的组合。Deployment 负责管理 Pod 副本和滚动更新策略；ClusterIP Service 为内部服务提供稳定的访问入口。Gateway 和 Frontend 使用 NodePort Service 暴露给外部访问。

### C.2.3 持久化存储

MySQL 使用 StatefulSet + PersistentVolumeClaim 实现数据持久化。PVC 可以绑定到本地存储或云盘，确保 Pod 重启或重新调度后数据不丢失。Redis 开启 AOF 持久化，Nacos 使用内置 Derby 数据库。

## C.3 监控告警配置

### C.3.1 Prometheus 抓取配置

Prometheus 通过 Kubernetes Service Discovery 自动发现目标服务。每个业务服务暴露了 `/actuator/prometheus` 端点，Prometheus 每隔 15 秒抓取一次指标数据。

### C.3.2 Grafana 看板

Grafana 中预设了三个核心看板：JVM 监控看板、HTTP 接口监控看板、业务自定义看板。HTTP 接口监控看板展示各接口的请求量、P99 延迟、错误率等关键指标。

### C.3.3 告警规则

```yaml
groups:
  - name: campus-studyroom-alerts
    rules:
...
**问题现象**：首次执行 `docker compose up --build` 时，多个服务卡在 `pulling base image` 阶段，最终超时失败。

**根因分析**：国内网络访问 Docker Hub 受限，默认镜像源连接不稳定。

**解决方案**：在 Docker Desktop 的 `daemon.json` 中配置多个国内镜像加速器，包括 daocloud、1ms、1panel、ustc 等。

**经验总结**：国内环境部署必须预先配置镜像加速器，建议配置多个源实现故障转移。

### C.4.2 Nacos 容器端口绑定失败

**问题现象**：Nacos 容器启动时报错 `bind: An attempt was made to access a socket in a way forbidden by its access permissions`。

**根因分析**：Windows 系统将 8835-8934 端口段标记为保留端口，Nacos 默认 8848 端口落入该区间。

**解决方案**：将 Nacos 宿主端口映射从 8848 改为 8758，容器内仍保持 8848。

**经验总结**：部署前需确认宿主端口是否被系统保留或已被占用。

### C.4.3 业务服务启动后反复崩溃

**问题现象**：业务服务启动后立即退出，日志显示 `spring.config.import` 触发 Nacos 配置中心但找不到对应 dataId。

**根因分析**：`spring.config.import: nacos:` 配置会主动连接 Nacos 配置中心，但配置中心中没有预先创建对应的 dataId，导致上下文初始化失败。

**解决方案**：在 `application-docker.yml` 中关闭 Nacos 配置中心（`spring.cloud.nacos.config.enabled=false`），仅保留服务发现功能。

**经验总结**：明确区分 Nacos 的服务发现与配置中心两个功能，按需启用。

### C.4.4 网关路由返回 503

**问题现象**：通过 Gateway 访问业务接口时，偶发返回 `503 Service Unavailable`。

**根因分析**：Gateway 使用 `lb://campus-xxx` 动态路由，但缺少 `spring-cloud-starter-loadbalancer` 依赖，无法解析 `lb://` 协议。

**解决方案**：在 campus-gateway 的 pom.xml 中增加 `spring-cloud-starter-loadbalancer` 依赖。

**经验总结**：使用 Spring Cloud Gateway + Nacos 时，LoadBalancer 是必需依赖。
...
| 服务注册 | Nacos 控制台服务列表 | 7 个微服务全部健康 |
| 接口连通性 | `curl http://localhost:8000/actuator/health` | `{"status":"UP"}` |
| 数据库连接 | 使用 MySQL 客户端连接 localhost:3307 | 连接成功 |
| Redis 连接 | `redis-cli -p 6380 ping` | 返回 PONG |
| K8s Pod 状态 | `kubectl get pods -n campus-studyroom` | 所有 Pod Running |
...
架构决策记录（Architecture Decision Records, ADR）用于记录项目中的重要架构决策、决策背景、备选方案以及最终选择。以下列出本项目的 5 条关键 ADR。

## ADR-001 采用微服务架构而非单体架构

**状态**：已接受

**背景**：项目涉及多个相对独立的业务领域，且需要两人并行开发。

**决策**：采用 Spring Cloud Alibaba 微服务架构，将系统拆分为 7 个独立微服务。

**后果**：正面：支持独立开发、独立部署、独立扩展；负面：增加了运维复杂度和服务间通信成本。

## ADR-002 采用 JWT 无状态认证而非 Session 有状态认证

**状态**：已接受

**背景**：微服务架构下，Session 需要集中存储，增加了状态管理复杂度。

**决策**：采用 JWT 无状态认证，由 Gateway 统一校验 Token。

**后果**：正面：服务无状态，便于水平扩展；负面：Token 过期和刷新逻辑需要前端额外处理。

## ADR-003 采用 MySQL + 达梦8 双数据库适配

**状态**：已接受

**背景**：课程要求完成国产化适配，响应国家信创战略。

**决策**：以 MySQL 8.0 为主数据库，同时提供达梦8 的建表脚本和 MyBatis-Plus 方言适配。

**后果**：正面：满足信创要求；负面：需要维护两套 SQL 脚本，增加了测试工作量。

## ADR-004 采用 Redis 分布式锁保障预约并发安全

**状态**：已接受

**背景**：同一座位同一时间段可能被多个用户同时预约，需要可靠的并发控制机制。

**决策**：采用 Redis SET NX EX 实现分布式锁，结合数据库唯一索引作为最终防线。

**后果**：正面：将并发冲突前移至应用层，降低数据库压力；负面：需要处理锁超时、锁释放失败等边界情况。

## ADR-005 采用 Python 独立服务实现 AI 算法

**状态**：已接受

**背景**：推荐算法和 RAG 检索涉及大量数值计算和 NLP 处理，Java 生态在该领域不如 Python 成熟。

**决策**：campus-ai 服务使用 Python 实现，通过 RESTful API 与 Java 微服务集成。

**后果**：正面：充分利用 Python 算法库；负面：增加了服务技术栈多样性，需要同时维护 Java 和 Python 两套环境。

## ADR-006 当前阶段不引入消息队列

**状态**：已接受

**背景**：项目周期有限，消息队列的引入会增加部署和测试复杂度。

**决策**：当前阶段采用应用内事件机制实现异步通知，预留 RabbitMQ 扩展点。

**后果**：正面：降低了当前阶段的复杂度；负面：服务间事件耦合度较高，未来需要通过消息队列解耦。
...
| 功能说明 | 用户使用学号和密码登录，成功后返回 JWT Token |
| 请求参数 | username: string, password: string |
| 响应数据 | accessToken: string, refreshToken: string, expiresIn: int |
| 异常响应 | 401：用户名或密码错误；400：参数校验失败 |

## E.2 创建预约接口

| 项目 | 内容 |
|------|------|
| 接口路径 | POST /api/reservation/reservations |
| 功能说明 | 学生选择座位和时段后提交预约请求 |
| 请求头 | Authorization: Bearer {accessToken} |
| 请求参数 | seatId: long, startTime: datetime, endTime: datetime |
| 响应数据 | reservationId: long, seatId: long, status: string, qrCode: string |
| 异常响应 | 409：座位时间冲突；403：信用积分不足 |

## E.3 查询自习室列表接口

| 项目 | 内容 |
|------|------|
| 接口路径 | GET /api/room/study-rooms |
| 功能说明 | 按条件查询自习室列表及座位可用状态 |
| 请求参数 | buildingId: long?, floor: int?, date: date? |
| 响应数据 | List<StudyRoomVO>，包含 roomId、roomName、building、floor、availableSeats |
| 缓存策略 | Redis 缓存 5 分钟 |
...
| 项目 | 内容 |
|------|------|
| 接口路径 | POST /api/ai/recommendations |
| 功能说明 | 基于用户历史行为和偏好生成自习室座位推荐 |
| 请求头 | Authorization: Bearer {accessToken} |
| 请求参数 | date: date?, building: string?, preferWindow: bool?, preferPower: bool? |
| 响应数据 | List<RecommendationVO>，包含 roomId、seatId、score、reason |
| 降级策略 | 推荐引擎异常时返回热门座位兜底 |
...
| 功能说明 | 接收用户自然语言问题，返回基于知识库的回答 |
| 请求参数 | message: string, history: List<Message>? |
| 响应数据 | reply: string, relatedDocs: List<DocRef> |
| 降级策略 | 大模型不可用时返回本地关键词匹配回答 |
...
《软件架构设计：大型网站技术架构与业务架构融合之道》一书由温昱编著，系统介绍了大型互联网系统的架构设计方法。本书提出的“业务架构与技术架构融合”思想对本项目的架构设计具有重要指导意义。我们在划分微服务边界时，参考了书中“按业务领域拆分、按限界上下文界定服务边界”的建议。

《领域驱动设计：软件核心复杂性应对之道》由 Eric Evans 编写，是 DDD 领域的经典著作。本书提出的实体、值对象、聚合、领域服务、限界上下文等概念，帮助我们更好地理解了预约域、考勤域、AI 域等领域对象的划分方式。

《微服务设计模式》由 Chris Richardson 编写，详细介绍了微服务架构中的各种设计模式，包括服务拆分、事务管理、查询模式、外部 API 模式等。本书对我们设计服务间通信、数据一致性和 API 网关具有直接参考价值。

## F.2 技术框架官方文档

Spring Cloud Alibaba 官方文档提供了 Nacos、Sentinel、Seata 等组件的详细使用说明。我们在配置 Gateway 路由、Nacos 服务注册、Sentinel 限流预留时，均参考了官方文档中的最佳实践。

Vue 3 官方文档详细介绍了组合式 API、响应式系统、生命周期钩子等核心概念。前端组件架构和状态管理设计主要依据官方文档的推荐模式。

达梦8 数据库管理员手册提供了达梦数据库与 MySQL 的语法差异对照。我们在编写达梦版 SQL 脚本和 MyBatis-Plus 方言适配器时，参考了手册中的数据类型映射和分页语法说明。

## F.3 开源社区与最佳实践

Kubernetes 官方文档介绍了 Deployment、Service、ConfigMap、StatefulSet 等资源的使用方法。我们在设计 K8s 部署拓扑和编写 YAML 配置文件时，遵循了官方文档中的推荐做法。

Prometheus 和 Grafana 官方文档提供了指标采集、存储、可视化的完整方案。虽然我们当前的可观测性组件处于设计预留阶段，但文档中的配置示例为后续实际部署提供了清晰的路径。

## F.4 本项目的学习价值

通过本次期末综合设计，我们不仅掌握了微服务架构、前后端分离、云原生部署等技术，还深刻体会到了架构设计在项目开发中的重要性。一个好的架构能够降低系统复杂度、提高开发效率、增强系统可维护性；而一个糟糕的架构则会导致代码耦合严重、部署困难、bug 频发。我们相信，本次项目积累的架构设计经验将对未来的学习和工作产生长远的积极影响。
